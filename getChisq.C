...
double getChisq(RooDataHist &dat, RooAbsPdf &pdf, RooRealVar &var, bool prt=false) {

  // Find total number of events
  double nEvt;
  double nTot=0.0;

  // should this be the expected events?(after fitting its the same
  for(int j=0;j<dat.numEntries();j++) {
    dat.get(j);
    nEvt=dat.weight();
    nTot+=nEvt;
  }
  nTot = pdf.expectedEvents(RooArgSet(var));
